...
def insertar_datos():
    ...
    try:
        for i in range(11):
            ...
            engine.execute(
                f""" INSERT INTO usuarios(nombre,telefono) values ('{nombre}','{telefono}')""")
...
@app.route('/createtabla', methods=["POST"])
def create_tabla():
    try:
        engine.execute(
            "CREATE TABLE IF NOT EXISTS usuarios(id serial, nombre varchar(200),telefono varchar(200))")
    except:
        return jsonify({"Respuesta": "FALLE"})
    return jsonify({"Respuesta": "Base de datos creada exitosamente"})
